...
export const useTypingStore = defineStore('typing-store',() => {
    ...
    const getWords = computed(() => {
            ...
            if(selectedMenuOption.value === 'word') {
                words.length = 0;
                words.push(...generateWords(word_constant.value,number.value,symbol.value).map((word) => word.val));
                return words;
            
            }else {
                words.length = 0;
                words.push(...generateWords(200,number.value,symbol.value).map((word) => word.val));
                return words;
            }
    ...
    const getErrorArr = computed(() => {
        return Object.values(testResult).map((res) => res.errors).filter((res) => res !== 0);
    })
    ...
    return {testResult,charStats,timePassed,getTimeArr,getWpmArr,getRawArr,getErrorArr,restart,
         status,symbol,number,time_constant,word_constant,difficulty,selectedMenuOption,getWords,words};
});
